...
struct deckState {
	...
	stack<card> drawDeck;
	card discard[45] = { 0,0 };																			// can this be made to be "card discard[deckSize];" in any way?
	int discardSize = 0;
};

int cardCount(int);																				// How many cards are in the game
void initializeDeck(deckState* deck);															// intializes the deck
...
void initializeDeck(deckState* deck) {									//this should be called once to set up the intial deck and never again
	int count = 0;

	for (int i = 0; i < cardCount(1); i++) {			//each of the for loops adds a type of card into the deck
		deck->discard[count] = { 1, 0 };					//this one adds the 1's to the deck
		count++;
	}
	for (int i = 0; i < cardCount(2); i++) {
		deck->discard[count] = { 2, 0 };
		count++;
	}
	for (int i = 0; i < cardCount(3); i++) {
		deck->discard[count] = { 3, 0 };
		count++;
	}
	for (int i = 0; i < cardCount(4); i++) {
		deck->discard[count] = { 4, 0 };
		count++;
	}
	for (int i = 0; i < cardCount(5); i++) {
		deck->discard[count] = { 5, 0 };
		count++;
	}
	for (int i = 0; i < cardCount(7); i++) {
		deck->discard[count] = { 7, 0 };
		count++;
	}
	for (int i = 0; i < cardCount(8); i++) {
		deck->discard[count] = { 8, 0 };
		count++;
	}
	for (int i = 0; i < cardCount(10); i++) {
		deck->discard[count] = { 10, 0 };
		count++;
	}
	for (int i = 0; i < cardCount(11); i++) {
		deck->discard[count] = { 11, 0 };
		count++;
	}
	for (int i = 0; i < cardCount(12); i++) {
		deck->discard[count] = { 12, 0 };
		count++;
	}
	for (int i = 0; i < cardCount(13); i++) {
		deck->discard[count] = { 13, 0 };
		count++;
	}
